  ...
  return (
    <>
      <Float speed={1.5} rotationIntensity={0.5} floatIntensity={0.5}>
        <Sphere ref={meshRef} args={[0.5, segments, segments]} position={[-2, 1, -2]}>
          <meshPhongMaterial color="#3B82F6" transparent opacity={0.6} />
        </Sphere>
      </Float>

      <Float speed={2} rotationIntensity={0.8} floatIntensity={0.8}>
        <Sphere args={[0.3, segments, segments]} position={[2, -1, -1]}>
          <meshPhongMaterial color="#8B5CF6" transparent opacity={0.4} />
        </Sphere>
      </Float>

      <Float speed={1.2} rotationIntensity={0.3} floatIntensity={0.3}>
        <Sphere args={[0.4, segments, segments]} position={[0, 2, -3]}>
...
export default function ThreeBackground() {
  ...
  return (
    <motion.div
      ...
      initial={{ opacity: 0 }}
      animate={{ opacity: isMobile ? 0.2 : 0.3 }}
